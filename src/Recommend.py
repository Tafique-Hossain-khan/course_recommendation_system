...
#clean_data = preprocessing.pre_process(df)
    ...
    def recommend_courses(df,text):
        ...
        #course_title,course_url = preprocessing.extract_fetures(final_recommended_course)
    ...
    def recommend_on_keyword(df,keyword):
        result_df = df[df['course_title'].str.contains(keyword)].sort_values(by='num_reviews',ascending=False).head(6)

        course_url = list(result_df['url'])
        course_title = list(result_df['course_title'])
